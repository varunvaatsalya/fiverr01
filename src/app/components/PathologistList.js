...
import AddPathologistSection from "./AddPathologistSection";
import Navbar from "./Navbar";
import Footer from "./Footer";
import { IoPersonAdd } from "react-icons/io5";

function AdminsList({ pathologists, setPathologists }) {
  const [newUserSection, setNewUserSection] = useState(false);

  return (
    <>
      {newUserSection ? (
        <AddPathologistSection
          setNewUserSection={setNewUserSection}
          setPathologists={setPathologists}
        />
      ) : (
        <></>
      )}
            ...
            {pathologists.map((pathologist, index) => {
              return (
                ...
                >
                  ...
                  <div className="flex items-center">{pathologist.name}</div>
                  <div className="w-2/5 flex items-center justify-center">{pathologist.email}</div>
                  <div className="w-2/5 flex items-center justify-center">
                    {pathologist.password}
                  </div>
                </div>
              );
            })}
