    ...
    <div className="w-full flex-1 min-h-0 px-2 text-gray-900 bg-slate-200">
      ...
      <div className=" overflow-x-auto flex-1 min-h-0 mt-1 bg-white shadow-md rounded-xl">
        <div className="bg-gray-100 flex items-center">
          {["name", "manufacturer", "salts", "netRevenue", "netStripsSold"].map(
            (col) => (
              <div
                key={col}
                onClick={() => handleSort(col)}
                className="py-2 px-4 text-left cursor-pointer select-none hover:bg-gray-200 font-semibold transition rounded-tl-xl flex items-center gap-2 flex-1 min-w-40"
              >
                {col.toUpperCase()}
                {sortConfig.key === col &&
        ...
        <div className="flex-1 min-h-0 text-sm overflow-y-auto">
          {sortedMedicines.map((med) => (
            <div
              key={med._id}
              className="border-t hover:bg-gray-50 transition flex"
            >
              <div className=" flex-1 min-w-40 py-2 px-4">{med.name}</div>
              <div className=" flex-1 min-w-40 py-2 px-4">
                {med.manufacturer}
              </div>
              <div className=" flex-1 min-w-40 py-2 px-4">
                {med.salts.join(", ")}
