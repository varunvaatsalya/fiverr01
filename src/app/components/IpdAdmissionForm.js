...
function IpdAdmissionForm({ bed, setBed, selectedSection, section }) {
  const sections = [
    { component: AddOns, props: { bed, setBed } },
    { component: AdmissionReason, props: { bed, setBed } },
    { component: InsuranceDetails, props: { bed, setBed } },
    { component: BedDetails, props: { bed } },
    { component: SupplementaryDetails, props: { bed } },
    { component: OtherServices, props: { bed, setBed } },
    { component: PaymentHistory, props: { bed } },
    { component: BalanceDetails, props: { bed } },
  ];
  const SelectedComponent = sections[selectedSection]?.component;
  const selectedProps = sections[selectedSection]?.props;

  return (
    <div className="flex flex-col h-full justify-between items-center gap-1 p-1">
      <div className="flex items-center justify-between gap-1 flex-wrap w-full text-sm md:text-base">
        <div className="px-4 py-1 bg-gray-800 text-white rounded-full font-semibold md:hidden">
          {section}
        </div>
        <div className="bg-gray-800 text-gray-100 flex justify-center gap-4 py-1 px-3 rounded-full">
          <div className="">
            Patient:{" "}
        ...
        <div className="bg-gray-800 text-gray-100 flex flex-wrap justify-center gap-x-4 py-1 px-3 rounded-full">
          <div className="">
            Admission ID:{" "}
...
function AddOns({ bed, setBed }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        } else showError("Error fetching data");
      } catch (err) {
  ...
  async function handleAddOns() {
    ...
    if (missingSurgeryDate || missingDoctorDate || missingPackageDate) {
      showError("Please select date & time for all selected items.");
      return;
    }
    try {
      setSubmitting(true);
  ...
  return (
    <div>
      ...
      <div
        className="w-3/4 mx-auto px-6 py-2 cursor-pointer border-b-2 border-gray-800 bg-slate-600 rounded-full hover:bg-gray-800 text-gray-100 flex justify-between items-center"
        onClick={() => {
      ...
      >
        ...
        <span className="text-gray-300">{activeAddOns === 0 ? "-" : "+"}</span>
      </div>
      {activeAddOns === 0 && (
      ...
      >
        ...
        <span className="text-gray-300">{activeAddOns === 1 ? "-" : "+"}</span>
      </div>
      {activeAddOns === 1 && (
        <div className="bg-slate-800 p-4 w-3/4 mx-auto my-1 rounded-xl">
      ...
      >
        ...
        <span className="text-gray-300">{activeAddOns === 2 ? "-" : "+"}</span>
      </div>
      {activeAddOns === 2 && (
        <div className="bg-slate-800 p-4 w-3/4 mx-auto my-1 rounded-xl">
...
function AdmissionReason({ bed, setBed }) {
  ...
  return (
    <div className="w-full flex flex-col items-center">
      <button
        disabled={
          submitting || !reason || reason === bed.occupancy.admissionId.reason
        }
        onClick={handleAddOns}
        className="text-lg bg-red-500 hover:bg-red-700 disabled:bg-gray-500 py-1 px-3 rounded-lg"
      >
        {submitting ? "Saving..." : "Save"}
      </button>
      <textarea
        placeholder="Reason for Admission"
        className="rounded-lg p-3 my-2 w-3/4 bg-slate-900 outline outline-gray-600"
        onChange={(e) => {
...
function InsuranceDetails({ bed, setBed }) {
  ...
  return (
    <div>
      <>
        <div className="w-full flex justify-center">
          <div className="flex flex-col md:flex-row my-2 justify-center items-center gap-2 w-4/5">
            <div className="flex-1">
              <label htmlFor="providerName" className="text-gray-800">
                Provider Name
              </label>
              <input
                type="text"
            ...
            <div className="flex-1">
              <label htmlFor="tpa" className="text-gray-800">
                TPA
              </label>
              <input
                type="text"
            ...
            <div className="flex-1">
              <label htmlFor="coverageAmount" className="text-gray-800">
                Coverage Amount
              </label>
              <input
                type="number"
        ...
        <div className="bg-slate-800 p-4 w-3/4 mx-auto my-1 rounded-xl">
          <div className="text-center text-lg font-semibold my-1">Payments</div>
  ...
  async function handleBedChange() {
    try {
      if (!changeingBedId || !changeingBedDate) {
        showError("Select the bed and date");
        return;
      }
      setSubmitting(true);
        ...
        {bedHistory ? (
            ...
            <div className="py-2 px-4 bg-slate-800 w-4/5 mx-auto my-2 rounded-xl">
              ...
              <div className="p-2 w-full md:w-4/5 mx-auto flex justify-center items-center gap-2 bg-slate-700 my-2 rounded-lg">
                ...
                <button
                  ...
                  disabled={!changeingBedId || !changeingBedDate || submitting}
          ...
          <>
            <div className="bg-slate-800 px-4 py-2 w-[90%] mx-auto my-1 flex flex-col items-center rounded-xl">
              <div className="text-center text-lg font-semibold my-1">
                {submitting ? "Fetching Bed Details..." : "Fetch Bed Details"}
              </div>
            </div>
          </>
        )}
...
function SupplementaryDetails({ bed }) {
  return (
    <div>
      <>
        <div className="bg-slate-800 p-4 w-3/4 mx-auto my-1 rounded-xl">
...
function OtherServices({ bed, setBed }) {
  const [submitting, setSubmitting] = useState(false);
  ...
  async function onOthServiceSubmit() {
    if (
      ...
      showError("fill the details correctly");
      return;
    }
    try {
      setSubmitting(true);
  ...
  return (
    <div className="bg-slate-800 p-4 w-3/4 mx-auto my-1 rounded-xl">
      ...
      bed.occupancy.admissionId.otherServices.length > 0 ? (
        ...
        <div className="text-center text-gray-400">No Any other services</div>
      )}
  ...
  return (
    <div className="bg-slate-800 p-4 w-3/4 mx-auto my-1 rounded-xl">
      ...
      bed.occupancy.admissionId.ipdPayments.length > 0 ? (
        ...
        <div className="text-center text-gray-400">No Payments Records</div>
      )}
    </div>
  );
}

function BalanceDetails({ bed }) {
  const router = useRouter();
        ...
        {chargeBalenceDetails ? (
          <>
            <div className="bg-slate-800 px-4 py-2 w-[90%] mx-auto my-1 rounded-xl">
              ...
              <div className="w-full px-3 py-1 border-b border-gray-600 mx-auto flex justify-around items-center text-sm">
                <div className="text-center w-full">Supplementary Charge</div>
                <div className="text-center w-full">
              ...
              <div className="w-full px-3 py-1 border-b-2 border-gray-600 mx-auto flex justify-around items-center text-sm">
                <div className="text-center w-full">Other Service Charge</div>
                <div className="text-center w-full">
            ...
            <div className="bg-slate-800 px-4 py-2 w-[90%] mx-auto my-1 rounded-xl">
              ...
              <div className="w-full px-3 py-1 border-b border-gray-600 mx-auto flex justify-around items-center text-sm">
                <div className="text-center w-full">IPD Invoice Payments</div>
                <div className="text-center w-full">
            ...
            <div className="py-2 px-4 bg-slate-800 w-4/5 mx-auto my-2 rounded-xl">
              ...
              <div className="p-2 w-full md:w-4/5 mx-auto flex justify-center items-center gap-2 bg-slate-700 my-2 rounded-lg">
                {bed.occupancy.admissionId?.insuranceInfo?.providerName ||
                chargeBalenceDetails.balance <= 0 ? (
                  <button
                    className="bg-red-500 hover:bg-red-700 px-4 py-1 rounded-lg"
                    onClick={() => {
                    ...
                    {bed.occupancy.admissionId?.insuranceInfo?.providerName &&
                      chargeBalenceDetails.balance > 0 && (
                        <div className="text-center w-3/4 mx-auto text-lg font-semibold text-red-500">
                          This patient is under insurance and the outstanding
