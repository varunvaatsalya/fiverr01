...
const ExpressInvoice = ({
  ...
  const [selectedDepartment, setSelectedDepartment] = useState(
    setEntity.department._id
  );
  const [availableItems, setAvailableItems] = useState([]);
  const [selectedItems, setSelectedItems] = useState(setEntity.items);
  const [initialDepartment, setInitialDepartment] = useState(selectedDepartment);

  useEffect(() => {
    setValue("patient", setEntity.patient?._id);
    setValue("department", setEntity.department._id);
    setValue("doctor", setEntity.doctor._id);
    setValue("items", setEntity.items);
  }, [setEntity]);
  ...
  const onSubmit = async (data) => {
    if (selectedItems.length > 0) {
      ...
      try {
        ...
        if (result.success) {
          deleteDataEntry(setEntity._id)
          setInvoiceGenerate(null);
  ...
  return (
    ...
    >
      ...
      <h2 className="font-bold text-2xl text-white">
        create <span className="text-blue-500">Prescription</span>
      </h2>
