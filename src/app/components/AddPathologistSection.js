...
function AddPathologistSection({ setNewUserSection, setPathologists }) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (result.success) {
        setPathologists((prevAdmins) => [result.pathologist, ...prevAdmins]);
          ...
          <div className="w-[95%] md:w-1/2 py-4 text-center bg-slate-950 px-4 rounded-xl">
            <div>
              <h2 className="font-bold text-2xl text-white">
                Details of new{" "}
                <span className="text-blue-500">pathologist</span>
              </h2>
              ...
              >
                <input
                  id="name"
                  type="text"
                  placeholder={"Enter the pathologist's name"}
...
export default AddPathologistSection;
